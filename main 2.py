...
# getting the accurate earth radius based on the latitude, considering the fact that the earth is not a perfect sphere
...
def calculate_speed_in_kmps(feature_distance, earth_GSD, time_difference, earth_radius, alt, alt_iss, min_dist):
    ...
    # accounting for the curvature of the earth and the iss's altitude
    distance = 2 * asin(distance / (2 * (earth_radius + alt))) * (earth_radius + alt_iss)
...
while time.time() - start_runtime < runtime:
    ...
    time_difference = get_time_difference(t1, t2) + 0.0007 # Get time difference between images, + constant accounting for the writing speed on the sd card
